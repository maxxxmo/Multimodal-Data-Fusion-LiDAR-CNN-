...
def decode_boxes(anchors, deltas):
    ...
    return torch.stack(
        [res_x, res_y, res_z, res_w, res_l, res_h, res_theta],
        dim=1
    )
...
def get_detected_boxes(cls_logits, reg_preds, anchor_generator, score_thresh=0.5):
    ...
    print("RAW anchors shape:", anchor_generator.anchors.shape)
    print("RAW anchors numel:", anchor_generator.anchors.numel())
